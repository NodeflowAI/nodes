...
class ColorizeTrain(QWidget):
    def __init__(self, nodeflow_main, n, nn, config):
        super(ColorizeTrain, self).__init__()

        DATASETS = config.get('DATASETS')
        CHECKPOINTS = config.get('CHECKPOINTS')

        name = 'temp_color'
        data_dir = DATASETS+'mini_colorization'

        nodeflow_main.createAttribute(node=n, name='Name', preset='String', socket=True, plug=True, dataType='str', dataAttr=name)
        nodeflow_main.createAttribute(node=n, name='Data_Path', preset='String', socket=True, plug=True, dataType='str', dataAttr=data_dir)
        ...
        nodeflow_main.createAttribute(node=n, name='Save_Latest_Freq', preset='Integer', socket=True, plug=True, dataType='int', dataAttr=5)
...
class ColorizeTrainAction(QAction):
    """Model Create"""
    def __init__(self, attrData, config):
        super(ColorizeTrainAction, self).__init__()
        ...
        command = 'python modules/PixGan/train.py --model colorization'+NAME+DATA+CPD+EPO+EPO_D+SLF+PF+DID+GPU
        self.run(command)
